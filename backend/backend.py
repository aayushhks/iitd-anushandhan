...
UPLOAD_FOLDER = r"/Users/aayushkumar/iitd-anusandhan/backend/static"
...
def filter_positive_compound_score(tokens):
    ...
    #     print("No negative words found after filtering.")
    #     return {}  # Return an empty dict instead of proceeding with an empty list
    word_counts = Counter(filtered_tokens)
    ...
    if request.method == 'POST':
        ...
        if text:
            ...
            if not word_frequencies:
                return jsonify({'status': 'error', 'message': 'No negative words found to create word cloud'})
...
def login_usr():
    try:
        ...
        if user['password'] != password:
            return jsonify({"error": "Invalid password!"}), 401  # Return 401 for invalid password
